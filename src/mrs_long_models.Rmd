...
#compare model fit
anova(lag_cre_age_TisFrac, lag_cre_age_TissFrac_sex) #not sig.
...
plot_lag_naa_age<-ggplot(lag_dat_plot, aes(x=mri_age_y, y=lag_naa_partial, na.exclude = TRUE, color=as.factor(female)))
plot_lag_naa_age + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, aes(x=mri_age_y, y=lag_naa_partial, color=as.factor(female), fill=as.factor(subj_id)), size = .2, show.legend = FALSE) +
...
plot_lag_cre_age<-ggplot(lag_dat_plot_Cre, aes(x=mri_age_y, y=lag_cre_partial, na.exclude = TRUE, color=as.factor(female)))
plot_lag_cre_age + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, aes(x=mri_age_y, y=lag_cre_partial, color=as.factor(female), fill=as.factor(subj_id)), size = .2, show.legend = FALSE) +
