...
write.csv(lag_dat, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_molal_n320_Feb2023.csv", row.names = FALSE)
...
#exclude Cho and Ins for PS14_046 due to issue in spectrum from visual QC
acg_dat[3, "Cho_GPC_PCh_conc_molal"]<- NA
acg_dat[3, "Ins_conc_molal"]<- NA
...
rosnerTest(acg_dat$Cho_GPC_PCh_conc_molal, k = 3) #no sig outliers
...
rosnerTest(acg_dat$Ins_conc_molal, k = 3) #outlier value 1.63, observation 84 (PS10168_PS18_013) - FID-A and LCModel borderline, CRLB val for Ins = 100%, other metabolites <=10%, so just excluding mI
acg_dat[82, "Ins_conc_molal"]<- NA
...
write.csv(acg_dat, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/acg_data_molal_outliers_removed_n112_Jan2023.csv", row.names = FALSE)
```
LAG
```{r}
...
plot_cre + geom_density()
plot_cre + geom_boxplot()
rosnerTest(lag_dat$Cre_PCr_conc_molal, k = 8) #outliers val. 15.76, obs. 241 (PS17_047) - structure in resid. & val. 14.82, obs. 178 (PS17_045, spectrum looked fine), excluding only PS17_047 for now
...
plot_cho + geom_density()
plot_cho + geom_boxplot()
rosnerTest(lag_dat$Cho_GPC_PCh_conc_molal, k = 6) #outlier value 3.37 obs. 170 (10090_PS15_015), coreg and spectrum look fine, CRLB low, keeping

plot_ins <- ggplot(lag_dat, aes(Ins_conc_molal))
plot_ins + geom_density()
plot_ins + geom_boxplot()
rosnerTest(lag_dat$Ins_conc_molal, k = 8) #outliers: val. 14.63, obs. 241 (PS17_047) - structure in resid; val. 12.139, obs. 221 (PS16_014) - spectrum fine, CRLB for Ins=3%, keep for now; val. 1.77, obs. 197 (PS18_028) - noisy spectrum, spiky residuals, exclude; obs. 139 (10080_PS15_144) looks fine, but CRLB=72% -exclude; obs. 62 (PS14_111) - spectrum looks ok, but CRLB for Ins=56%; obs. 223, (PS17_012) spectrum ok, but CRLB for Ins = 76%, exclude
lag_dat["241", "Ins_conc_molal"] <- NA #replace outlier value with NA to exclude from analysis
lag_dat["197", "Ins_conc_molal"] <- NA
lag_dat["139", "Ins_conc_molal"] <- NA
lag_dat["62", "Ins_conc_molal"] <- NA
lag_dat["223", "Ins_conc_molal"] <- NA
...
write.csv(lag_dat, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_molal_outliers_removed_n318_Feb2023.csv", row.names = FALSE)
```
...
#write to csv
write.csv(acg_metab_desc, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/acg_metab_descriptives_Feb2023.csv")
...
#write to csv
write.csv(lag_metab_desc, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_metab_descriptives_n318_Feb2023.csv")
...
write.csv(fdr_table_acg, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/fdr_table_acg_age_Feb2023.csv")
```
...
acg_summary_table_all<-tab_model(acg_naa_age, acg_cho_age, acg_cre_age, acg_glx_age, acg_ins_age, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("tNAA (molal)", "tCho (molal)", "tCr (molal)", "Glx (molal)", "Ins (molal)"), pred.labels = c("(Intercept)", "Age (years)"))
...
## Add plots for non-sig metabolites (Cr, Glx, Ins)
...
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/acg_ins_age_Feb2023.png")
...
acc_fig_all
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/manuscript/figures/acc_all_fig_annot_feb2023_vertical.png", acc_fig_all, width = 2000, height = 3000, units = "px")
```
...
lag_summary_table_all<-tab_model(lag_naa_age_TisFrac, lag_cho_age_TisFrac, lag_cre_age_TisFrac,  lag_glx_age_TisFrac, lag_ins_age_TisFrac, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("tNAA (molal)",  "tCho (molal)", "tCr (molal)", "Glx (molal)", "Ins (molal)"), pred.labels = c("(Intercept)", "Age (years)", "Tissue Fraction GM/(WM+GM)"))
...
write.csv(fdr_table_lag_new, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/fdr_table_lag_feb2023.csv")
```
...
lag_naa<-ggplot(lag_dat_plot, aes(x=mri_age_y, y=lag_naa_partial, na.exclude = TRUE, color=as.factor(female))) +
  ...
  labs(x = "Age (years)", y = "tNAA (residual)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  ...
  theme(legend.position='none')
lag_naa
ggsave(plot=lag_naa,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_naa_age_tf_feb2023.png")
...
lag_cre<-ggplot(lag_dat_plot_Cre, aes(x=mri_age_y, y=lag_cre_partial, na.exclude = TRUE, color=as.factor(female))) +
  ...
  labs(x = "Age (years)", y = "tCr (residual)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  ...
  theme_classic(base_size=14) +
  theme(legend.position = c(.9,.98), legend.title = element_blank(),legend.background = element_blank(), legend.box.background = element_rect(colour = "black"), legend.text = element_text(size=14), legend.key.height = unit(.3, 'cm'), legend.key.width=unit(.3, 'cm'), legend.spacing.y = unit(0, "pt"))
lag_cre
ggsave(plot=lag_cre,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_cre_age_tf_feb2023.png")
...
lag_cho<-ggplot(lag_dat_plot_cho, aes(x=mri_age_y, y=lag_cho_partial, na.exclude = TRUE, color=as.factor(female))) +
  ...
  labs(x = "Age (years)", y = "tCho (residual)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  geom_abline(aes(intercept = fixef.lag_cho_age_TisFrac[[1]], slope = fixef.lag_cho_age_TisFrac[[2]]), size=1) + 
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
  scale_y_continuous(limits=c(1.5,4.5), breaks=seq(1,5,1)) +
  theme_classic(base_size=14) +
  theme(legend.position = 'none')
lag_cho
ggsave(plot=lag_cho,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_cho_age_tf_feb2023.png")
```

## add plots for non-sig effects per reviewer request
Glx & mI
```{r}
#LAG - Glx
#pull fixed effects for overall model
fixef.lag_glx_age_TisFrac<-fixef(lag_glx_age_TisFrac)

#subset dataframe with only subjects included in Cre analysis
lag_dat_plot_glx<- lag_dat %>% 
...
lag_glx<-ggplot(lag_dat_plot_glx, aes(x=mri_age_y, y=lag_glx_partial, na.exclude = TRUE, color=as.factor(female))) +
  geom_point(size=1) +
  geom_smooth(method = "lm", se = FALSE, aes(x=mri_age_y, y=lag_glx_partial, group=as.factor(subj_id)), size = .5, show.legend = FALSE) +
  labs(x = "Age (years)", y = "Glx (residual)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
  scale_y_continuous(limits=c(6,19), breaks=seq(7,19,2)) +
  theme_classic(base_size=14) +
  theme(legend.position = 'none')
lag_glx
ggsave(plot=lag_glx,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_glx_age_tf_feb2023.png")
...
lag_ins<-ggplot(lag_dat_plot_ins, aes(x=mri_age_y, y=lag_ins_partial, na.exclude = TRUE, color=as.factor(female))) +
  ...
  labs(x = "Age (years)", y = "mI (residual)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
  ...
  theme(legend.position = 'none')
lag_ins
ggsave(plot=lag_ins,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_ins_age_tf_feb2023.png")

```


## Join LTP plots into figure
```{r}
library(patchwork)
...
#version including all plots per reviewer request
lag_fig_all<-lag_naa + lag_cho + lag_cre + lag_glx + lag_ins + guide_area() + plot_layout(nrow=3, guides = 'collect') + plot_annotation(tag_levels = 'A')
lag_fig_all
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/manuscript/figures/ltp_all_fig_annot_feb2023_vertical.png", lag_fig_all, width = 2000, height = 3000, units = "px")
...
write.csv(lag_dat_long, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_3plus_visits_Feb2023.csv")
...
write.csv(lag_long_desc,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_long_metab_means_feb2023.csv")
```
...
lag_long_summary_table_all<-tab_model(lag_naa_TisFrac_age_long,  lag_cho_TisFrac_age_long_quad, lag_cre_TisFrac_age_long, lag_glx_TisFrac_age_long_quad, lag_ins_TisFrac_age_long, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("tNAA (molal)",  "tCho (molal)", "tCr (molal)", "Glx (molal)", "mI (molal)"), pred.labels = c("(Intercept)", "Age (years)", "GM Tissue Fraction", "Age^2"))
...
```
## FDR Correction for longitudinal models
```{r}
#FDR correction
#list models
...
p_long_age<-c(p_long, p_quad_age, p_quad_age2)

model_names<-c("NAA", "Cre", "Ins","Cho", "Glx", "Cho_quad", "Glx_quad")
...
write.csv(fdr_table_long_age, "/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/fdr_table_long_age_feb2023.csv")
...
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_naa_long_age_feb2023.png", plot=plot_lag_naa_tf_long)
...
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_cho_long_quad_feb2023.png", plot=plot_lag_cho_tf_quad)
...
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_cre_long_feb2023.png", plot=plot_lag_cre_tf_long)
...
plot_lag_glx_tf_quad<-ggplot(lag_dat_long_glx, aes(x=mri_age_y, y=lag_glx_age_long_partial, na.exclude = TRUE)) +
  ...
  geom_smooth(method = "lm", formula = y ~ poly(x,2), se = FALSE, colour = "black", size=1) + 
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
  scale_y_continuous(limits=c(7,17), breaks=seq(7,17,1)) +
  theme_classic(base_size=14)+
  theme(legend.position= 'none')
plot_lag_glx_tf_quad
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_glx_long_quad_feb2023.png", plot=plot_lag_glx_tf_quad)
...
#plot mI (non-significant)
#subset dataframe with only subjects included in tCre analysis
lag_dat_long_ins<- lag_dat_long %>% 
  filter(! is.na(Ins_conc_molal))

#calculate partial effect tissue fraction on tCre (remove effect of tissue fraction)
lag_ins_age_long_partial <- remef(lag_ins_TisFrac_age_long, fix = "TissueFraction_GMWM") 

plot_lag_ins_tf_long<-ggplot(lag_dat_long_ins, aes(x=mri_age_y, y=lag_ins_age_long_partial, na.exclude = TRUE)) +
  geom_point(aes(color=as.factor(female)),size=1) +
  geom_smooth(method = "lm", se = FALSE, aes(x=mri_age_y, y=lag_ins_age_long_partial, group=as.factor(subj_id), color=as.factor(female)), size = .5, show.legend = FALSE) +
  labs(x = "Age (years)", y = "mI (residual)") +
  scale_color_viridis(name="Sex",labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
...
## Join longitudinal plots into figure
```{r}
...
#version including all plots per reviewer request
lag_long_fig_all<-plot_lag_naa_tf_long + plot_lag_cre_tf_long + plot_lag_cho_tf_quad + plot_lag_glx_tf_quad + plot_lag_ins_tf_long + guide_area() + plot_layout(nrow=3, guides = 'collect') + plot_annotation(tag_levels = 'A')
lag_long_fig_all
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/manuscript/figures/ltp_long_all_fig_annot_feb2023_vertical.png", lag_long_fig_all, width = 2000, height = 3000, units = "px")
...
write.csv(acg_pct.df,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/acg_pct_chg_feb2023.csv", row.names=FALSE)
...
write.csv(lag_pct.df,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_pct_chg_feb2023.csv", row.names=FALSE)
...
```{r}
acg_dat<- read.csv("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/acg_data_molal_outliers_removed_n112_Jan2023.csv")
...
lag_dat<-read.csv("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_molal_outliers_removed_n318_Feb2023.csv")
...
plot_lag_snr<-ggplot(lag_dat, aes(x=mri_age_y, y=SNR_NAA_LCM, na.exclude = TRUE)) +
  geom_point(aes(color=as.factor(female)),size=1) +
  geom_smooth(method = "lm", se = FALSE, aes(x=mri_age_y, y=SNR_NAA_LCM, group=as.factor(subj_id), color=as.factor(female)), size = .5, show.legend = FALSE) +
    geom_smooth(method = "lm", se = FALSE, colour = "black", size=1) + 
  labs(x = "Age (years)", y = "SNR of NAA") +
  scale_color_viridis(name="Sex",labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
...
ggsave("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/LTP_Age_SNR.png", plot_lag_snr, width = 1600, height = 1600, units = "px")
...
#create model summary tables for SI
ACC_QCage_summary_table_all<-tab_model(acg_age_snr, acg_age_lw, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("ACC - SNR", "ACC - Linewidth"), pred.labels = c("(Intercept)", "Age (years)"))
ACC_QCage_summary_table_all

LTP_QCage_summary_table_all<-tab_model(lag_age_snr, lag_age_lw, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("LTP - SNR", "LTP - Linewidth"), pred.labels = c("(Intercept)", "Age (years)"))
LTP_QCage_summary_table_all


#add SNR to metabolite-age models to see if it influences effects
...
summary(lag_cre_age_TisFrac_snr) #age effect no longer sig. (p = .118)
...
lag_QCmetab_summary_table_all<-tab_model(lag_naa_age_TisFrac_snr,  lag_cho_age_TisFrac_snr, lag_cre_age_TisFrac_snr, lag_glx_age_TisFrac_snr, lag_ins_age_TisFrac_snr, show.std = TRUE, show.re.var=FALSE, show.icc = FALSE, show.r2=FALSE, dv.labels = c("tNAA (molal)",  "tCho (molal)", "tCr (molal)", "Glx (molal)", "mI (molal)"), pred.labels = c("(Intercept)", "Age (years)", "GM Tissue Fraction", "SNR"))
lag_QCmetab_summary_table_all #opens in Rstudio viewer, can open from there to web browser and save as PDF or whatever
...
acg_dat<-read.csv("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/acg_data_molal_outliers_removed_n112_Jan2023.csv")
lag_dat<-read.csv("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_molal_outliers_removed_n318_Feb2023.csv")
lag_dat_long<-read.csv("/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/data/lag_data_3plus_visits_Feb2023.csv")
