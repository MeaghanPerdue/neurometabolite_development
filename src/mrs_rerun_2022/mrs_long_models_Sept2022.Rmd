...
plot_cho + geom_density()
plot_cho + geom_boxplot()
rosnerTest(lag_dat$Cho_GPC_PCh_conc_molal, k = 6) #outlier value 3.37 obs. 170 (10090_PS15_015), coreg and spectrum look fine, keeping
...
lag_glx<-ggplot(lag_dat_plot_glx, aes(x=mri_age_y, y=lag_glx_partial, na.exclude = TRUE, color=as.factor(female))) +
  ...
  labs(x = "Age (years)", y = "Glx (corrected for tissue fraction)") +
  scale_color_viridis(name = "Sex", labels = c("Male", "Female"), discrete = TRUE, option = "H", begin = .2, end = .8) +
  scale_x_continuous(limits=c(2,12), breaks=seq(2,12,1)) +
  scale_y_continuous(limits=c(6,19), breaks=seq(6,19,1)) +
  theme_classic(base_size=14) +
  theme(legend.position = 'none')
lag_glx
...
lag_ins<-ggplot(lag_dat_plot_glx, aes(x=mri_age_y, y=lag_ins_partial, na.exclude = TRUE, color=as.factor(female))) +
  geom_point(size=1) +
  ...
  theme(legend.position = 'none')
lag_ins
ggsave(plot=lag_ins,"/Users/meaghanperdue/OneDrive - University of Calgary/Preschool_data/MRS/results/lag_cho_age_tf_feb2023.png")
