...
Count number of scans per participant per voxel & write to df
```{python}
mrs_lag_ses_counts=mrs_lag.value_counts(subset='subj_id').reset_index(name='counts')
mrs_lag_ses_counts.to_csv("/Users/meaghan/OneDrive - University of Calgary/Preschool_data/MRS/data/mrs_lag_ses_counts.csv", index=False)
mrs_acg_ses_counts=mrs_acg.value_counts(subset='subj_id').reset_index(name='counts')
mrs_acg_ses_counts.to_csv("/Users/meaghan/OneDrive - University of Calgary/Preschool_data/MRS/data/mrs_acg_ses_counts.csv", index=False)
```
...
mrs_lag.to_csv("/Users/meaghan/OneDrive - University of Calgary/Preschool_data/MRS/data/mrs_lag_withpaths.csv", index=False)
...
mrs_acg = pd.read_csv("/Users/meaghan/OneDrive - University of Calgary/Preschool_data/MRS/data/mrs_data_ACG_Oct2021.csv")

mrs_acg['pfile_prefix'] = "SPECT-EXAM"
mrs_acg['sep'] = "-"

mrs_acg['pfile_path'] = mrs_acg['pfile_prefix'].map(str) + mrs_acg['spec_id'].astype(str).str.split('.', expand = True)[0] + mrs_acg['sep'].map(str) + mrs_acg['date_scan_dt'].astype(str).str.split(' ', expand = True)[0]
